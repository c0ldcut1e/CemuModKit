// TODO: Modify Cemu to handle crash logs in a better way so this here actually works
...
static const char *sCrashType = NULL;

static char sDisassemblyBuffer[LOG_DISASSEMBLY_SIZE];

static uint32_t sDisassemblyLength = 0;

static char sStackTraceBuffer[LOG_STACK_TRACE_SIZE];

static uint32_t sStackTraceLength = 0;

static char sRegistersBuffer[LOG_REGISTER_SIZE];

static uint32_t sRegistersLength = 0;

static uint8_t sCrashThreadStack[THREAD_STACK_SIZE];

static OSThread __attribute__((aligned(8))) sCrashThread;

static int crashReportThread(int argc, const char **argv)
{
    (void) argc;
    (void) argv;
    // Log crash dump
...
static CUT_FORMAT_PRINTF(1, 2) void disassemblyPrintCallback(const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
static void getDisassembly(OSContext *context)
{
    ...
    if (context->srr0 > 16)
    {
        DisassemblePPCRange((void *) (context->srr0 - 16), (void *) (context->srr0 + 16), disassemblyPrintCallback, OSGetSymbolName,
                            DISASSEMBLE_PPC_FLAGS_NONE);
    }
}

static void getStackTrace(OSContext *context)
{
    int i;
    uint32_t *stackPtr;
    char name[256];
    ...
    for (i = 0; i < 16; ++i)
    {
        ...
        sStackTraceLength += sprintf(sStackTraceBuffer + sStackTraceLength, "0x%08x:   0x%08x    0x%08x", (uintptr_t) stackPtr,
                                     (uintptr_t) stackPtr[0], (uintptr_t) stackPtr[1]);

        addr = OSGetSymbolName(stackPtr[1], name, sizeof(name));
        if (addr)
        {
            sStackTraceLength += sprintf(sStackTraceBuffer + sStackTraceLength, " %s+0x%x", name, (uintptr_t) (stackPtr[1] - addr));
        }
...
static CUT_FORMAT_PRINTF(1, 2) void writeRegister(const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
static void getRegisters(OSContext *context)
{
...
static BOOL handleException(const char *type, OSContext *context)
{
    ...
    OSCreateThread(&sCrashThread, crashReportThread, 0, NULL, sCrashThreadStack + THREAD_STACK_SIZE, THREAD_STACK_SIZE, 16, 0);
...
static BOOL handleAlignment(OSContext *context) { return handleException("ALIGNMENT", context); }

static BOOL handleDSI(OSContext *context) { return handleException("DSI", context); }

static BOOL handleISI(OSContext *context) { return handleException("ISI", context); }

static BOOL handleProgram(OSContext *context) { return handleException("PROGRAM", context); }
