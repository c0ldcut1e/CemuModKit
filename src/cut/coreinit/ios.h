...
CUT_CHECK_OFFSET(IOSVec, 0x00, vaddr);
CUT_CHECK_OFFSET(IOSVec, 0x04, len);
CUT_CHECK_OFFSET(IOSVec, 0x08, paddr);
CUT_CHECK_SIZE(IOSVec, 0x0C);

typedef void (*IOSAsyncCallbackFn)(IOSError, void *);

CUT_IMPORT IOSError (*IOS_Open)(const char *device, IOSOpenMode mode);

CUT_IMPORT IOSError (*IOS_Close)(IOSHandle handle);

CUT_IMPORT IOSError (*IOS_Ioctl)(IOSHandle handle, uint32_t request, void *inBuf, uint32_t inLen, void *outBuf, uint32_t outLen);

CUT_IMPORT IOSError (*IOS_IoctlAsync)(IOSHandle handle, uint32_t request, void *inBuf, uint32_t inLen, void *outBuf, uint32_t outLen,
                                      IOSAsyncCallbackFn callback, void *context);

CUT_IMPORT IOSError (*IOS_Ioctlv)(IOSHandle handle, uint32_t request, uint32_t vecIn, uint32_t vecOut, IOSVec *vec);

CUT_IMPORT IOSError (*IOS_IoctlvAsync)(IOSHandle handle, uint32_t request, uint32_t vecIn, uint32_t vecOut, IOSVec *vec, IOSAsyncCallbackFn callback,
                                       void *context);
