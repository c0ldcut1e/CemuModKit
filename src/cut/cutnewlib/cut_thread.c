...
static OSFastMutex *__cut_get_mutex(_LOCK_T *lock)
{
    if (*lock)
    {
...
static OSFastCondition *__cut_get_cond(_COND_T *cond)
{
    if (*cond)
    {
...
void __SYSCALL(lock_acquire)(_LOCK_T *lock) { OSFastMutex_Lock(__cut_get_mutex(lock)); }

int __SYSCALL(lock_try_acquire)(_LOCK_T *lock) { return OSFastMutex_TryLock(__cut_get_mutex(lock)) ? 0 : 1; }

void __SYSCALL(lock_release)(_LOCK_T *lock) { OSFastMutex_Unlock(__cut_get_mutex(lock)); }
...
int __SYSCALL(lock_try_acquire_recursive)(_LOCK_RECURSIVE_T *lock) { return __SYSCALL(lock_try_acquire)(&lock->lock); }
...
{
    OSFastCond_Signal(__cut_get_cond(cond)); // XX: this is broadcast
    return 0;
}

int __SYSCALL(cond_broadcast)(_COND_T *cond)
{
    OSFastCond_Signal(__cut_get_cond(cond));
    return 0;
}
...
static void __cut_cond_alarmcb(OSAlarm *alarm, OSContext *context)
{
    (void) context;
    __cut_condwait_t *data = OSGetAlarmUserData(alarm);
    data->timed_out        = true;
...
int __SYSCALL(cond_wait)(_COND_T *cond, _LOCK_T *lock, uint64_t timeout_ns)
{
    ...
    {
        OSFastCond_Wait(__cut_get_cond(cond), __cut_get_mutex(lock));
        return 0;
    }

    __cut_condwait_t data;
    data.cond      = __cut_get_cond(cond);
    data.timed_out = false;

    OSAlarm alarm;
    OSCreateAlarm(&alarm);
    OSSetAlarmUserData(&alarm, &data);
    OSSetAlarm(&alarm, OSNanosecondsToTicks(timeout_ns), __cut_cond_alarmcb);
    OSFastCond_Wait(data.cond, __cut_get_mutex(lock));
    OSCancelAlarm(&alarm);
...
int __SYSCALL(thread_create)(struct __pthread_t **thread, void *(*func)(void *), void *arg, void *stack_addr, size_t stack_size)
{
    ...
    BOOL ok = OSCreateThread(&t->base, (OSThreadEntryPointFn) func, (int) arg, NULL, stack_addr, stack_size, 16, OS_THREAD_ATTRIB_AFFINITY_ANY);
...
void __SYSCALL(thread_exit)(void *value) { OSExitThread((int) value); }

struct __pthread_t *__SYSCALL(thread_self)(void) { return (struct __pthread_t *) OSGetCurrentThread(); }
...
int sched_getcpu(void) { return OSGetCoreId(); }
