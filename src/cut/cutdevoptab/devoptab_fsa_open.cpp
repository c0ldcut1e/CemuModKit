...
int __cut_fsa_open(struct _reent *r, void *fileStruct, const char *path, int flags, int mode)
{
    ...
    file       = (__cut_fsa_file_t *) fileStruct;
    ...
    {
        OSReport("__cut_fsa_open: snprintf result was truncated\n");
    }
    free(fixedPath);
    ...
    if (createFileIfNotFound || failIfFileNotFound || (flags & (O_EXCL | O_CREAT)) == (O_EXCL | O_CREAT))
    {
        ...
        if (status == FS_ERROR_NOT_FOUND)
        {
            ...
            { // Create new file if needed
                status = FSAOpenFileEx(deviceData->clientHandle, file->fullPath, "w", translatedMode, openFlags, preAllocSize, &fd);
                if (status == FS_ERROR_OK)
                {
                    if (FSACloseFile(deviceData->clientHandle, fd) != FS_ERROR_OK)
                    {
                        OSReport("FSACloseFile(0x%08X, 0x%08X) (%s) failed: %s\n", deviceData->clientHandle, fd, file->fullPath,
                ...
                {
                    OSReport("FSAOpenFileEx(0x%08X, %s, %s, 0x%X, 0x%08X, 0x%08X, %p) failed: %s\n", deviceData->clientHandle, file->fullPath, "w",
                             translatedMode, openFlags, preAllocSize, &fd, FSAGetStatusStr(status));
                    r->_errno = __cut_fsa_translate_error(status);
                    return -1;
                }
            }
            else if (failIfFileNotFound)
            { // Return an error if we don't we create new files
                r->_errno = __cut_fsa_translate_error(status);
                return -1;
            }
        }
    ...
    if (status < 0)
    {
        if (status != FS_ERROR_NOT_FOUND)
        {
            OSReport("FSAOpenFileEx(0x%08X, %s, %s, 0x%X, 0x%08X, 0x%08X, %p) failed: %s\n", deviceData->clientHandle, file->fullPath, fsMode,
                     translatedMode, openFlags, preAllocSize, &fd, FSAGetStatusStr(status));
        }
        r->_errno = __cut_fsa_translate_error(status);
        return -1;
