...
off_t __cut_fsa_seek(struct _reent *r, void *fd, off_t pos, int whence)
{
    FSError status;
    FSAStat fsStat;
    uint64_t offset;
    ...
    if (status < 0)
    {
        OSReport("FSASetPosFile(0x%08X, 0x%08X, 0x%08X) (%s) failed: %s\n", deviceData->clientHandle, file->fd, file->offset, file->fullPath,
                 FSAGetStatusStr(status));
        file->offset = old_pos;
        r->_errno    = __cut_fsa_translate_error(status);
        return -1;
