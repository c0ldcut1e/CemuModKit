...
int __cut_fsa_dirnext(struct _reent *r, DIR_ITER *dirState, char *filename, struct stat *filestat)
{
    ...
    if (status < 0)
    {
        ...
        {
            OSReport("FSAReadDir(0x%08X, 0x%08X, %p) (%s) failed: %s\n", deviceData->clientHandle, dir->fd, &dir->entry_data, dir->fullPath,
    ...
    if (fullStr)
    {
        ...
        {
            OSReport("__cut_fsa_dirnext: snprintf fullStr result was truncated\n");
        }
        ino = __cut_fsa_hashstring(fullStr);
        free(fullStr);
    }
    else
    {
        ino = 0;
        OSReport("__cut_fsa_dirnext: Failed to allocate memory for fullStr. st_ino will be "
                 "set to 0\n");
    }
    __cut_fsa_translate_stat(deviceData->clientHandle, &dir->entry_data.info, ino, filestat);

    if (snprintf(filename, NAME_MAX, "%s", dir->entry_data.name) >= NAME_MAX)
    {
        OSReport("__cut_fsa_dirnext: snprintf filename result was truncated\n");
    }
