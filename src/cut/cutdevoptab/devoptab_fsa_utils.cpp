...
static char *__cut_fsa_normpath(char *out, const char *in)
{
...
char *__cut_fsa_fixpath(struct _reent *r, const char *path)
{
    ...
    if (!fixedPath)
    {
        OSReport("__cut_fsa_fixpath: failed to allocate memory for fixedPath\n");
        r->_errno = ENOMEM;
        return NULL;
    }
    ...
    if (p[0] != '\0' && p[0] != '\\' && p[0] != '/')
    {
        __cut_fsa_device_t *deviceData = (__cut_fsa_device_t *) r->deviceData;
        if (snprintf(fixedPath, maxPathLength, "%s/%s", deviceData->cwd, p) >= maxPathLength)
        {
            OSReport("__cut_fsa_fixpath: fixedPath snprintf result (absolute) was truncated\n");
        }
    }
    else
    {
        ...
        {
            OSReport("__cut_fsa_fixpath: fixedPath snprintf result (relative) was truncated\n");
        }
    }
    ...
    if (!normalizedPath)
    {
        OSReport("__cut_fsa_fixpath: failed to allocate memory for normalizedPath\n");
        free(fixedPath);
        r->_errno = ENOMEM;
        return NULL;
    }

    char *resPath = __cut_fsa_normpath(normalizedPath, fixedPath);
    if (!resPath)
    {
        OSReport("__cut_fsa_fixpath: failed to normalize path\n");
        free(normalizedPath);
        free(fixedPath);
        r->_errno = EIO;
        return NULL;
...
void __cut_fsa_translate_stat(FSAClientHandle clientHandle, FSStat *fsStat, ino_t ino, struct stat *posStat)
{
    memset(posStat, 0, sizeof(struct stat));
    posStat->st_dev     = (dev_t) clientHandle;
    posStat->st_ino     = ino;
    posStat->st_mode    = __cut_fsa_translate_stat_mode(fsStat);
    posStat->st_nlink   = 1;
    posStat->st_uid     = fsStat->owner;
    posStat->st_gid     = fsStat->group;
    posStat->st_rdev    = posStat->st_dev;
    posStat->st_size    = fsStat->size;
    posStat->st_atime   = __cut_fsa_translate_time(fsStat->modified);
    posStat->st_ctime   = __cut_fsa_translate_time(fsStat->created);
    posStat->st_mtime   = __cut_fsa_translate_time(fsStat->modified);
